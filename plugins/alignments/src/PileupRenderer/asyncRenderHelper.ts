...
import { RenderArgsDeserialized as BoxRenderArgsDeserialized } from '@jbrowse/core/pluggableElementTypes/renderers/BoxRendererType'
...
export default async function doRender(
  ...
  const regionSequence =
    features.size && shouldFetchReferenceSequence(renderProps.colorBy?.type)
      ? await fetchSequenceHelper(renderProps, pluginManager)
      : undefined
...
function layoutFeats(props: RenderArgsDeserializedWithFeaturesAndLayout) {
...
function layoutFeature({
  ...
  const l = feature.get('start') - expansionBefore
  const r = feature.get('end') + expansionAfter
  const [leftPx, rightPx] = bpSpanPx(l, r, region, bpPerPx)
  ...
  const topPx = layout.addRect(feature.id(), l, r, heightPx, feature)
  if (topPx === null) {
    return null
  }
...
async function fetchSequenceHelper(
  renderProps: RenderArgsDeserialized,
  pm: PluginManager,
) {
...
function colorByOrientation(feature: Feature, config: AnyConfigurationModel) {
  return alignmentColoring[getOrientation(feature, config) || 'color_nostrand']
}
...
function colorByInsertSize(feature: Feature, _config: AnyConfigurationModel) {
...
function colorByStranded(feature: Feature, _config: AnyConfigurationModel) {
...
function colorByPerBaseLettering({
  ctx,
  feat,
  region,
  bpPerPx,
...
function colorByPerBaseQuality({
  ctx,
  feat,
  region,
  bpPerPx,
...
function drawAlignmentRect({
  ...
  switch (colorType) {
    case 'insertSize':
      ctx.fillStyle = colorByInsertSize(feature, config)
      break
    case 'strand':
    ...
    case 'pairOrientation':
      ctx.fillStyle = colorByOrientation(feature, config)
      break
    case 'stranded':
      ctx.fillStyle = alignmentColoring[colorByStranded(feature, config)]
      break
    case 'xs':
    case 'tag': {
